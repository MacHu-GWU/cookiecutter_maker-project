...
def test_replace_double_curly_brackets():
    text = """
    f"{{ \"{name}\" }}".format(name="Alice")
    """.strip()
    text = replace_double_curly_brackets(text)
    assert (
        text
        == 'f"{% raw %}{{{% endraw %} "{name}" {% raw %}}}{% endraw %}".format(name="Alice")'
    )
...
if __name__ == "__main__":
    ...
    run_cov_test(
        __file__,
        "cookiecutter_maker.str_replace",
        preview=False,
    )
